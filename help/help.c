...
{
  char digest[33];
  help_checksum_md5(HelpDocDir, digest);
...
/**
 * help_file_type - Determine the type of a help file (relative to HelpDocDir)
...
static HDType help_file_type(const char *file)
{
  ...
  const size_t m = mutt_str_strlen(HelpDocDir);
  ...
  if ((mutt_str_strncasecmp(q, ".md", 3) != 0) ||
      (mutt_str_strncmp(file, HelpDocDir, m) != 0))
    return type; /* path below HelpDocDir and ".md" extension are mandatory */
  ...
  if (p == q)
    type |= HDT_ROOTDOC; /* help document lives directly in HelpDocDir root */
...
{
  va_list ap;
  char subject[STRING]; /* XXX: Should trailing white space (from strfmt) be removed? */
...
static void *help_doc_clone(const void *item)
{
  ...
  dup->env->from = mutt_addr_copy(src->env->from);
...
static struct Email *help_doc_from(const char *file)
{
  ...
  const char *rfp = (file + mutt_str_strlen(HelpDocDir) + 1);
  ...
  hdoc->env->from =
      mutt_addr_parse_list(NULL, "Richard Russon <rich@flatcap.org>");
...
int help_doclist_init(void)
{
  ...
  help_dir_read(HelpDocDir);
  help_docdir_id(HelpDocDir);

  return help_dir_scan(HelpDocDir, true, DET_DIR, NULL, help_dir_gather, NULL);
}
 ...
 */
static int help_doclist_parse(struct Context *ctx)
{
  if ((help_doclist_init() != 0) || (DocList->size == 0))
    return -1;

  ctx->mailbox->emails = (struct Email **) (help_list_clone(DocList, true, help_doc_clone))->data;
  ctx->mailbox->msg_count = ctx->mailbox->email_max = DocList->size;
  mutt_mem_realloc(&ctx->mailbox->v2r, sizeof(int) * ctx->mailbox->email_max);

  mutt_make_label_hash(ctx->mailbox);

  ctx->mailbox->readonly = true;
  /* all document paths are relative to HelpDocDir, so no transpose of ctx->path */
  mutt_str_strfcpy(ctx->mailbox->realpath, mutt_str_strdup(HelpDocDir), sizeof(ctx->mailbox->realpath));

  /* check (none strict) what the user wants to see */
  const char *request = help_path_transpose(ctx->mailbox->path, false);
  ctx->mailbox->emails[0]->read = false;
  if (request)
  {
    mutt_str_strfcpy(ctx->mailbox->path, help_path_transpose(request, false), sizeof(ctx->mailbox->path)); /* just sanitise */
    request += mutt_str_strlen(HelpDocDir) + 1;
    for (size_t i = 0; i < ctx->mailbox->msg_count; i++)
    { /* TODO: prioritise folder (chapter/section) over root file names */
      if (mutt_str_strncmp(ctx->mailbox->emails[i]->path, request, mutt_str_strlen(request)) == 0)
      {
        ctx->mailbox->emails[0]->read = true;
        ctx->mailbox->emails[i]->read = false;
        break;
      }
    }
  }
...
static int help_mbox_open(struct Mailbox *m, struct Context *ctx)
{
  ...
  {
    if (access(HelpDocDir, F_OK) == 0)
    { /* ensure a proper path, especially without any trailing slashes */
      mutt_str_replace(&HelpDocDir, NONULL(realpath(HelpDocDir, NULL)));
    }
    else
    {
      mutt_debug(1, "unable to access help mailbox '%s': %s (errno %d).\n",
                 HelpDocDir, strerror(errno), errno);
      return -1;
    }
  }

  __Backup_HTS = HideThreadSubject; /* backup the current global setting */
  HideThreadSubject = false; /* temporarily ensure subject is shown in thread view */

  return help_doclist_parse(ctx);
}

static int help_mbox_open_append(struct Mailbox *m, int flags)
{
  mutt_debug(1, "entering help_mbox_open_append\n");
  return -1;
}

static int help_mbox_check(struct Context *ctx, int *index_hint)
{
  mutt_debug(1, "entering help_mbox_check\n");
  return 0;
}

static int help_mbox_sync(struct Context *ctx, int *index_hint)
{
...
static int help_mbox_close(struct Mailbox *m)
{
  ...
  HideThreadSubject = __Backup_HTS; /* restore the previous global setting */
